...
async fn main() -> anyhow::Result<()> {
    ...
    let addr = std::env::var("PROXY_LISTEN")
        .unwrap_or_else(|_| "0.0.0.0:8443".to_string());
    let listener = TcpListener::bind(&addr).await?;
    info!("RDCleanPath proxy listening on {addr}");
...
async fn handle_connection(stream: TcpStream, peer: SocketAddr) -> anyhow::Result<()> {
    ...
    let (destination, x224_request) = match rdcleanpath {
        ...
        _ => {
            ...
            let err_bytes = err_pdu.to_der().map_err(|e| anyhow!("DER encode error: {e}"))?;
            ws_write.send(Message::Binary(err_bytes.into())).await?;
            return Err(anyhow!("Expected RDCleanPath Request, got something else"));
        }
    };
    ...
    // Step 4: Perform TLS handshake with the RDP server
    // First send the X.224 Connection Request (before TLS)
    ...
    // Read X.224 Connection Confirm (variable length TPKT: first 4 bytes are header)
    ...
    // Extract hostname from destination (host:port -> host)
    ...
    let response_bytes = response_pdu.to_der().map_err(|e| anyhow!("DER encode error: {e}"))?;
    ws_write
        ...
        while let Some(msg) = ws_read.next().await {
            match msg {
                ...
                Ok(Message::Ping(data)) => {
                    // Can't send pong from here easily, just ignore
                    let _ = data;
                }
